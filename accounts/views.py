...
def user_profile_display(request, user_username):
    ...
    user_info = {}
    user_info['tweets_data'] = Tweet.objects.filter(user_id = profile_basics.id).order_by("-timestamp")
    user_info['tweet_count'] = user_info['tweets_data'].count()
    user_info['mentions'] = Tweet.objects.filter(Q(content__icontains='@'+str(profile_basics.username)) & ~Q(user = profile_basics)).order_by("-timestamp")
